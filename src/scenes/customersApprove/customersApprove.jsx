...
const CustomersApprove = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(process.env.REACT_APP_API_URL+`/api/webcustomer/not-approve/list/?page=${page}`);
  ...
  const handleApprove = async (id) => {
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+`/api/webcustomer/approve/${id}/`, {
        method: "POST",
        headers: {
...
export default CustomersApprove;
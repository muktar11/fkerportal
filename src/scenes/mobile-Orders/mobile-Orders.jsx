...
const RemoteOrder = () => {
  ...
  const [cso_return_issue, setcso_return_issue] = useState("");
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(process.env.REACT_APP_API_URL+`commerce/sales-order-remote-view`);
        if (!response.ok) {
...
const handleQpChange = (event) => {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+`/commerce/remote-sales-order/reject/${id}/`, {
        method: "PUT",
        body: formData,
      });
      const data = await response.json();
  ...
  const handleApprove = async (id) => {
    try {
      ...
      const response = await fetch(process.env.REACT_APP_API_URL+`/commerce/remote-sales-order-approve-view/${id}/`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          cso_remote_approve_first_name: firstName,
          cso_remote_approve_last_name: lastName,
        }),
      });
      const data = await response.json();
      console.log("Sales Order Has Approved Successfully:", data);
      toast.success("Sales Order Has Approved Successfully:");
      handleUpdateClick()
      handleCloseModal()
      // You can update the customerData state or perform any other action here
    } catch (error) {
  ...
  const handleReject = async (id) => {
    try {
      ...
      const response = await fetch(process.env.REACT_APP_API_URL+`/commerce/remote-sales-order/reject/${id}/`, {
        ...
        body: JSON.stringify({
          cso_return_issue: cso_return_issue,
          cso_remote_approve_first_name: firstName,
          cso_remote_approve_last_name: lastName,
        }),
      });
      ...
      toast.error("Error returning Sales Order");
    }
  };
...
<div style={{ textAlign: 'center', padding: '10px' }}>
    <Header
      title="Remote Sales Order"
      subtitle="Remote Order Detail that requires Approval"
    />
  </div>
  ...
  <TableCell>0.35 Total Value</TableCell>
  <TableCell>
    <input
...
<TableRow  display="column">
<TableCell>Return Issue Case</TableCell>
  <TableCell>
    <input
      type="text"
      placeholder="state an issue if there is any..."
      value={cso_return_issue}
...
<TableRow style={{dispaly:"flex", justifyContent:"space-between"}}>
  ...
  <TableCell align="right">
      <Button
        variant="contained"
        color="primary"
        onClick={() => handleUpdate(selectedRow._id)}
        style={{
          borderRadius: "5px",

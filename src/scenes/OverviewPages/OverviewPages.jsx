...
const label = { inputProps: { 'aria-label': 'Switch demo' } };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(process.env.REACT_APP_API_URL+"/commerce/sales-order/timeframe/<str:start_date>/<str:end_date>/");
  ...
  const handleAADDSalesChanges = async (event) => {
    ...
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+`AADD-sales-order/timeframe/<str:start_date>/<str:end_date>/`);
      const data = await response.json();
  ...
  const handleAreaChanges = async (event) => {
    ...
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+ `/commerce/sales-orders/timeframe/${selectedArea}/<str:sales_route>/<str:start_date>/<str:end_date>/`);
      const data = await response.json();
  ...
  const handleDirectAreaChanges = async (event) => {
    ...
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+ `/commerce/AADD-sales-orders/timeframe/<int:sales_person_id>/<str:start_date>/<str:end_date>/`);
      const data = await response.json();
      setSalesPersonData(data.sales_orders);
    } catch (error) {
      console.error("Error fetching customer data:", error);
  ...
  const handleCustomerChanges = async (event) => {
    ...
    try {
      const response = await fetch(process.env.REACT_APP_API_URL+`/commerce/sales-orders/timeframe/<int:customer_id>/<str:start_date>/<str:end_date>/`);
      const data = await response.json();
      setCustomerData(data.sales_orders);
    } catch (error) {
      console.error("Error fetching customer data:", error);
...
<Box height="15vh">
  <Header  subtitle="Chart of Annually sales by Customer" />
  ...
  <Box height="15vh">
  <Header  subtitle="Chart of  Annually sales by SalesPerson" />
  <Box display="flex" alignItems="center">
...
overflow="hidden">
      ...
      <div className="body-title" style={{ textAlign: "center", width:"267mm", marginTop:"10px"  }}>
        <h2>  Annually AdissAbaba and UPC SalesReport</h2>
      </div>
  ...
  {isSwitchOn ? (
 <div class="table-container" style={{ display:"table",  marginTop:"10px" }}>
  ...
  <div class="table-container" style={{ display:"table", width:"100%", marginTop:"10px" }}>
    <div class="table-row table-header" style={{display: "table-row" }}>
      ...
      <div class="table-cell" style={{display: "table-cell", padding: "5px", border:"1px solid black", width:"5%" }}>SalesPerson</div>

...
def create_Raw_Material(request):
    ...
    fields_allow_empty_string = [
        ...
        'FG_Standardized_2l', 'FG_Standardized_Total', 'FG_Damaged_035ml','FG_Damaged_06ml',
        'FG_Damaged_1l','FG_Damaged_2l','FG_Damaged_Total',
    ]
...
def access_raw_material(request, pk):
    raw = RawMaterialRequest.objects.filter(recipant_store=pk, is_await=True)
...
@api_view(['GET'])
def access_raw_material(request, pk):
    raw = RawMaterialRequest.objects.filter(Q(recipant_store=pk) & (Q(is_approved=False)))
...
def accept_raw_material(request, pk):
    data = request.data 
    raw = RawMaterialRequest.objects.get(_id=pk)
    raw.issue_store_file = data['recipant_store_file']
